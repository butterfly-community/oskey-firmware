...
#define ZEPHYR_PROJECT_USB_VID		0x1209
...
USBD_DEVICE_DEFINE(sample_usbd,
		   ...
		   ZEPHYR_PROJECT_USB_VID, CONFIG_OSKEY_USBD_PID);
/* doc device instantiation end */

/* doc string instantiation start */
USBD_DESC_LANG_DEFINE(sample_lang);
USBD_DESC_MANUFACTURER_DEFINE(sample_mfr, CONFIG_OSKEY_USBD_MANUFACTURER);
USBD_DESC_PRODUCT_DEFINE(sample_product, CONFIG_OSKEY_USBD_PRODUCT);
...
static const uint8_t attributes = (IS_ENABLED(CONFIG_OSKEY_USBD_SELF_POWERED) ?
				   USB_SCD_SELF_POWERED : 0) |
				  (IS_ENABLED(CONFIG_OSKEY_USBD_REMOTE_WAKEUP) ?
				   USB_SCD_REMOTE_WAKEUP : 0);

/* Full speed configuration */
USBD_CONFIGURATION_DEFINE(sample_fs_config,
			  attributes,
			  CONFIG_OSKEY_USBD_MAX_POWER, &fs_cfg_desc);

/* High speed configuration */
USBD_CONFIGURATION_DEFINE(sample_hs_config,
			  attributes,
			  CONFIG_OSKEY_USBD_MAX_POWER, &hs_cfg_desc);
